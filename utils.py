...
def get_selection_with_quit(conn, options, quit_option='q'):
    option_idx = [x for x in range(1, len(options) + 1)]
    recv_msg = conn.recv(100).decode("utf-8")
    while recv_msg != quit_option and int(recv_msg) not in option_idx:
        msg = "[INPUT]Invalid input, please select "
        for idx in option_idx:
            msg += f'[{idx}] '
        msg += f'or enter [{quit_option}] to quit: '
        conn.send(msg.encode('utf-8'))
        recv_msg = conn.recv(100).decode("utf-8")
    
    if recv_msg == quit_option:
        return None
    return options[int(recv_msg) - 1]
